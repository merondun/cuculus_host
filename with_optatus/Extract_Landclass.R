...
#Extract points
md = read_tsv('~/merondun/cuculus_host/with_optatus/Metadata_256.txt')
md = md %>% filter(Host_Optatus == 1) %>% select(ID, Latitude, Longitude)
...
pdf('../with_optatus/figures/LandClass_ValuesCounts_SINUproj_2024FEB23.pdf',height=4,width=6)
lcp
dev.off()
lc2 = cbind(md,lc %>% select(-ID))

#save it 
write.table(lc2,file='../with_optatus/LandClass_Designations_2024FEB23.txt',quote=F,sep='\t',row.names=F)
...
#save the points
st_write(points_sf_transformed, "Cuckoo_CanOpt_Locations_sinu.shp",delete_layer = TRUE)

# Plot to verify
pdf('../with_optatus/figures/LandClass_Values_SINUproj_2024FEB23.pdf',height=5,width=9)
plot(clipped_raster)
