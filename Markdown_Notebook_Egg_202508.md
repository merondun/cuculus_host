...
## Plot Tesselation + mtDNA Pies [Fig. 1C & 1D]
...
host_leg <- md %>% drop_na(HostParentShort) %>% distinct(HostParentShort) %>% 
  mutate(HostCol = rep_len(viridis(6,option='turbo'),25),
         HostShape = rep_len(c(21,22,23,24,25),25)) 
  #%>% count(HostCol,HostShape) %>%  # to check
  #filter(n > 1) #to check 
host_leg %>% count(HostCol,HostShape) %>% filter(n > 1) #to check 

host_colors <- host_leg %>% select(HostParentShort,HostCol) %>% deframe()
host_shapes <- host_leg %>% select(HostParentShort,HostShape) %>% deframe()
...
plothost = ggplot() +
  geom_polygon(data = world, aes(x = long, y = lat, group = group), col = 'grey50', fill = 'grey95') +
  geom_sf(data = ksites, aes(fill = HostParentShort, shape = HostParentShort), size = 2, show.legend = TRUE) +
  ...
  annotation_scale(line_width = 0.5)
plothost
ggsave('~/symlinks/host/figures/20250508_HostLocations.pdf',plothost,dpi=300,height=4,width=6)


#### Host proportions divided by covariates 
ep = md %>% dplyr::rename(Ancestry=AncestryA5,Geography=GeographicGroup,Host=HostParentShort) %>% 
  drop_na(Host,Ancestry,Geography,Haplogroup) %>% 
  group_by(Host,SpeciesShort) %>% mutate(Total = n()) %>%
  pivot_longer(c(Ancestry,Geography,Haplogroup)) %>% ungroup %>%
  group_by(Host,SpeciesShort,name,value) %>%
  summarize(Parts = n(),
            Proportion = Parts / Total,
            Percent = paste0(round( Parts / Total,3)*100,'% (',Total,')')) %>% unique

# Bind them together
ap = rbind(ep %>% ungroup %>% mutate(Response = Host, variable = 'Host') %>% select(-Host))

# Just for ordering the covariates nicely
# Plot proportions
ord <- ap %>% select(name, SpeciesShort,value) %>%
  distinct() %>%
  mutate(ord = as.numeric(gsub("[^0-9.]", "", value))) %>%
  arrange(name,SpeciesShort, ord)
ap$value <- factor(ap$value,levels=ord$value)
app = ap %>%
  arrange(value) %>%
  ggplot(aes(y=Response,x=value,fill=Proportion,label=Parts))+
  facet_grid(SpeciesShort~name,scales='free',space='free')+
...
ggsave(paste0('~/symlinks/host/figures/20250508_HostProportions.pdf'),app,height=4,width=6,dpi=300)

```



## Distance Correlations [Fig. 1B & 1E & 1F]
